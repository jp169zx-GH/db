...
function formatBirthDisplay() {
  if (state.birthYear && state.birthMonth && state.birthDay) {
    const mm = String(state.birthMonth).padStart(2, "0");
    const dd = String(state.birthDay).padStart(2, "0");
    return `生日：${state.birthYear} 年 ${mm} 月 ${dd} 日`;
  }
  return "";
}
...
function renderProfileStep() {
  ...
  c.innerHTML = `
    ...
    <div class="field">
      ...
      <div class="birth-row">
        <input type="number" inputmode="numeric" id="birth-year" placeholder="年 (YYYY)" min="1900" max="2026" value="${state.birthYear ?? ""}" />
        <input type="number" inputmode="numeric" id="birth-month" placeholder="月 (MM)" min="1" max="12" value="${state.birthMonth ?? ""}" />
        <input type="number" inputmode="numeric" id="birth-day" placeholder="日 (DD)" min="1" max="31" value="${state.birthDay ?? ""}" />
      </div>
      <div class="birth-display" id="birth-display">${formatBirthDisplay()}</div>
    </div>
    <div class="field">
      <label>星座</label>
  ...
  const yearInput = c.querySelector("#birth-year");
  const monthInput = c.querySelector("#birth-month");
  const dayInput = c.querySelector("#birth-day");
  const displayEl = c.querySelector("#birth-display");
  const hintEl = c.querySelector("#zodiac-hint");
  const nextBtn = c.querySelector("#next-btn");
  ...
  const handleBirthInput = () => {
    ...
    const validYear = Number.isInteger(y) && y >= 1900 && y <= 2026;
    const validMonth = Number.isInteger(m) && m >= 1 && m <= 12;
    const daysInMonth = validMonth ? new Date(y || 2000, m, 0).getDate() : 31;
    const validDay = Number.isInteger(d) && d >= 1 && d <= daysInMonth;

    state.birthYear = validYear ? y : null;
    state.birthMonth = validMonth ? m : null;
    state.birthDay = validDay ? d : null;
    ...
    displayEl.textContent = formatBirthDisplay();
    const zodiacSelectEl = c.querySelector("#zodiac-select");
    zodiacSelectEl.value = state.zodiac || "";
    hintEl.textContent = state.zodiac ? "已选择：" + state.zodiac : "";
    nextBtn.disabled = !(state.gender && state.zodiac);
  };

  yearInput.oninput = handleBirthInput;
  monthInput.oninput = handleBirthInput;
  dayInput.oninput = handleBirthInput;
...
function renderResult() {
  ...
  c.querySelector("#restart-btn").onclick = () => {
    state.step = 0;
    state.gender = null; state.zodiac = null; state.birthYear = null; state.birthMonth = null; state.birthDay = null;
